...
mejor_arbol <- rpart(
  formula = popularity ~ duration_min + explicit + danceability + energy + key + loudness + mode + speechiness + acousticness + instrumentalness + liveness + valence + tempo + time_signature, 
  data = test_set, 
  method = "class", 
  control = rpart.control(
    maxdepth = 12, 
...
```{r}
test_set_mejor_arbol = test_set

popularidad_predicha  <- predict(mejor_arbol, newdata = test_set_mejor_arbol, type = "class")

test_set_mejor_arbol$predicted_class_popularity <- popularidad_predicha

names(test_set_mejor_arbol)[names(test_set_mejor_arbol) == "predicted_class_popularity"] <- "popularidad_predicha"

library(pROC)
vector_predicciones = as.numeric(test_set_mejor_arbol$popularidad_predicha)
curva_roc = roc(test_set_mejor_arbol$popularity, vector_predicciones)
...
plot(curva_roc, main = "Curva de AUC-ROC", col = "cyan", lwd = 3)
```
