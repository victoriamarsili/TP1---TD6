...
Obtenido el mejor resultado del modelo, podemos guardarnos estos en una variable para utilizarlos a futuro cuando querramos entrenar un modelo con estos híper-parámetros y evaluarlo con el test_set.
...
Establecer un min_bucket más alto generalmente mejora el rendimiento del modelo al incrementar el AUC-ROC y al reducir la variabilidad en el rendimiento, sugiriendo una mayor robustez y menor riesgo de sobreajuste. Sin embargo, el aumento en rendimiento se modera a medida que minbucket se incrementa más allá de 8 a 10, sugiriendo que puede haber un punto de rendimiento óptimo en ese rango para este hiperparámetro. 

Deduciendolo del gráfico, la mejora en AUC-ROC a medida que aumenta minbucket parece ser gradual, con un aumento constante pero no dramático. 
...
El árbol inicial es relativamente simple y tiene pocos niveles de profundidad. Habíamos observado que la primera división se realiza en la variable *instrumentalness* con un umbral de 0.0033.Posteriormente, se divide en *speechiness* y luego en *energy* y *acousticness.* Este árbol se enfoca principalmente en variables como instrumentalness, speechiness, energy, y acousticness para clasificar la popularidad.

En cambio, el árbol final es mucho más complejo y profundo, con múltiples niveles y nodos que abarcan una gran cantidad de variables. Muestra una mayor cantidad de divisiones y considera más combinaciones de variables para hacer la clasificación. A diferencia del árbol inicial, el árbol final toma en cuenta más variables y utiliza umbrales de clasificación más detallados y específicos.
...
datos_numerica$popularity <- as.numeric(as.character(datos_numerica$popularity))
...
De acuerdo con el árbol final y la matriz de correlación, las variables más importantes son:
    ...
    -*instrumentalness*: Esta variable fue la primera en dividir el árbol inicial y también aparece en el árbol final,        lo que sugiere que es una de las variables más influyentes en la clasificación de la popularidad. La correlación        negativa con la popularidad (-0.1507) indica que a medida que aumenta la instrumentalness, la popularidad tiende a       disminuir.
    
    -*speechiness*: También es una variable clave que se utiliza en las primeras divisiones del árbol inicial. Aunque         tiene una correlación negativa débil con la popularidad (-0.1036), su importancia en la estructura del árbol            indica que es relevante para la clasificación.
    
    -*energy*: Aparece tanto en el árbol inicial como en el final. Tiene una correlación cercana a cero con la                popularidad (-0.0301), pero su aparición en el árbol sugiere que, en combinación con otras variables, juega un          papel en la predicción.
    
    -*acousticness*: Se encuentra en ambos árboles, y su correlación negativa con la popularidad (-0.0035) es baja, pero        puede ser un factor determinante cuando se combina con otras variables en el árbol.
...
Ahora vamos a entrenar 3 modelos nuevos, cada uno con su correspondiente cantidad faltante de datos. Reutilizamos el código ya creado en el ejercicio 5 para simplificar el código. Pero primero tenemos que encontrar los híper-parámetros que mejor performance dan en el dataset de validación.
...
# Buscamos híper-parámetros óptimos para:
...
Ya con los híper-parámetros correspondientes, podemos entrenar nuestros nuevos modelos.

```{r}
# Ahora creamsos los 3 modelos utilizando los híper-parámetros encontrados
...
predecir_y_calcular_score <- function(arbol, set_test) {
  ...
  set_test$predicciones_optimas <- ifelse(test_20_na$predicciones_optimas >= 0.5, 1, 0)
  set_test$predicciones_optimas <- as.factor(test_20_na$predicciones_optimas)
...
graficar_curvas_ROC(curva_optimizada,curva_roc_20_NA, "Curva árbol 20 NA", "Curva árbol optimizado")

```

Este modelo arrojo un resultado peor al mejor árbol, pero no tan alejado de el mismo, sin embargo, esto tiene sentido, ya que únicamente faltan el 20% de los datos con los que se entreno, valido y testeo el modelo inicial. Con el modelo original obtuvimos un valor AUC-ROC de 0.68, mientras que con el 20% de los datos faltantes obtuvimos uno de 0.646.
...
graficar_curvas_ROC(curva_optimizada,curva_roc_50_NA, "Curva árbol 50 NA", "Curva árbol optimizado")

```

Ahora el modelo con el 50% de los datos faltantes arroja un valor AUC-ROC aún peor que el anterior, de tan solo 0.608, no es extremadamente malo, pero ya no es tan efectivo como quizas el anterior. Tiene sentido este resultado, porque ahora el modelo se entrenó y validó con un 50% de datos menos que el mejor modelo.
...
graficar_curvas_ROC(curva_optimizada,curva_roc_75_NA, "Curva árbol 75 NA", "Curva árbol optimizado")

```

Por último el modelo con el 75% de los datos faltantes. Es congruente que haya arrojado un valor inferior (de 0.57) al de 20% y 50% de datos faltantes, ya que en esta instancia el modelo se entrenó y validó con 1/4 del total de los datos que lo hizo el mejor modelo obtenido.
...
Quisimos llevar mas allá la investigación con los datos faltantes, por lo que analizamos que parte de los datos faltantes hacían que el modelo arroje peores resultados. Por lo que tomamos los modelos pre entrenados con datos faltantes y los testeamos con el data set completo: 
...
score_modelo_20_NA_con_set_completo

score_modelo_50_NA_con_set_completo

score_modelo_75_NA_con_set_completo
```

Para nuestra sorpresa, descubirmos que cuando testeamos a los modelos entrenados con datos faltantes, con un set de datos completo, el score obtenido no es tan negativo e incluso no cambia mucho entre modelos. 


###***8. Conclusiones y discusión***
Luego de entrenar el modelo optimizado, logramos predecir si una canción será popular (según el umbral definido por nosotros) utilizando únicamente las características disponibles en el dataset de Spotify. Aunque el modelo alcanzó un nivel de precisión de 0.837, los resultados no fueron tan satisfactorios como esperábamos. 

Al mejorar el modelo mediante la optimización de los hiperparámetros, logramos incrementar el área bajo la curva AUC-ROC en aproximadamente un 9%. Esto se consiguió a través de una optimización bayesiana con 1000 iteraciones.
...
Por lo tanto, sugerimos explorar otros tipos de modelos, como regresión logística, máquinas de vectores de soporte (SVM)
o redes neuronales, que podrían ofrecer un mejor rendimiento en la predicción de la popularidad de canciones, dadas las
características del dataset. Este cambio de enfoque podría mejorar considerablemente la precisión y la utilidad del modelo.
