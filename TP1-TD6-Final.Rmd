...
CONSULTAR CON OTRO GRUPO

### **Preparacion de los Datos**
Lo primero que hacemos es cargar el dataset de canciones y verificar si hay valores NA en el mismo. Aunque, cabe mencionar que los árboles de decisión pueden manejar datos con valores faltantes sin necesidad de imputación previa.

```{r}
library(readr)
spotify_data  = read.csv("spotify_data.csv")
datos <- subset(spotify_data, select = -track_genre)

table(is.na(spotify_data))
```
Para tener una visión completa de los datos con los que trabajaremos hemos verificado que no existen datos faltantes en el dataset.

Una vez cargado, empieza la etapa de limpieza de los datos. Por ejemplo, lo primero que notamos fue que el dataset contenia muchas repeticiones de canciones por lo que eliminamos dichas observaciones repetidas, disminuyendo la cantidad total de canciones de 114000 a 73609. Posterior a eso, procedemos a eliminar las variables "track_id", "artists" "album_name" y "track_name" dado que no son relevantes como variables predictoras para el trabajo práctico.
...
Por último, convertimos las observaciones de la variable "duration_ms" (que indica la duración de una canción expresada en milisegundos) de milisegundos a minutos. Además, renombramos la variable como "duration_min"
...
ggplot(muestra, aes(x = instrumentalness, y = speechiness)) +
...
correlacion <- cor(muestra$instrumentalness, muestra$speechiness)
...
```
Nos preguntábamos cómo definir correctamente el umbral para establecer que una canción sea popular o no.

```{r}
hist(muestra$popularity, main = "Histograma de popularidad", xlab = "popularidad", ylab = "frecuencia", breaks = 100, ylim = c(0, 1500))
abline(v = mean(muestra$popularity), col = "red", lwd = 5)
abline(v = median(muestra$popularity), col = "orange", lwd = 5)
```
...
```{r}
ESTO LO CAMBIA MARIANOOOOOOOOOOOOOOO
library(caret)
library(recipes)
...
Proseguimos a construir esta matriz para podert interpretar visualmente sus resultados.
```{r}
library(pheatmap)
...
A partir de la matriz de confusión podemos determinar: 
  - *TP*: 0.56 -> Proporción de verdaderos positivos en relación con el total de verdaderos positivos.
  - *TN*: 0.35 -> Proporción de verdaderos negativos en relación con el total de verdaderos negativos.
  - *FP*: 0.44 -> Proporción de falsos positivos en relación con el total de verdaderos negativos.
  - *FN*: 0.65 -> Proporción de falsos negativos en relación con el total de verdaderos positivos.
...
```
El modelo tiene una tasa de *accuracy* del 61.12%. Esta métrica indica que el modelo clasifica correctamente el 61.12% de las observaciones en el conjunto de datos. Esto significa que 6 de cada 10 predicciones realizadas por el modelo son correctas.
...
print(sprintf("Precisión: %.2f", precision))
print(sprintf("Recall: %.2f", recall))
```
Un valor de *precisión* de 0.60 (o 60%) indica que cuando el modelo predice la clase positiva, el 60% de estas predicciones son correctas. En otras palabras, de todas las predicciones realizadas como positivas, el 60% de ellas son realmente correctas. Este valor sugiere que el modelo tiene un desempeño moderado en términos de evitar falsos positivos, es decir, hay margen de mejora para minimizar los errores en los casos en que predice la clase positiva.

Un valor de *recall* de 0.69 (o 69%) indica que el modelo ha identificado correctamente el 69% de los verdaderos positivos. En otras palabras, de todos los casos que realmente pertenecen a la clase positiva, el modelo ha detectado el 69% de ellos. Aunque el valor de recall es razonablemente alto, un 31% de los verdaderos positivos no están siendo identificados por el modelo, lo que sugiere que hay espacio para mejorar la capacidad del modelo para detectar la clase positiva.
...
```
Un valor de F1 Score de 0.64 (o 64%) indica el equilibrio entre la precisión y el recall del modelo. En este caso, un valor de 0.64 sugiere que el modelo tiene un rendimiento moderado en términos de equilibrio entre evitar falsos positivos (precisión) y detectar verdaderos positivos (recall). Un F1 Score de 0.64 muestra que el modelo tiene margen de mejora para aumentar tanto la precisión como el recall, y así lograr un mejor rendimiento general.
...
# Por último medimos su AUC-ROC en test_set
predicciones_modelo_inicial = predict(arbol, newdata = test_set, type = "prob")[,2]

test_set$predicciones_iniciales = predicciones_modelo_inicial
test_set$predicciones_iniciales <- ifelse(test_set$predicciones_iniciales >= 0.5, 1, 0)
test_set$predicciones_iniciales <- as.factor(test_set$predicciones_iniciales)

curva_default <- roc(test_set$popularity, predicciones_modelo_inicial)
auc_score_arbol_inicial <- auc(curva_default)
auc_score_arbol_inicial

# Plotea la curva ROC
plot(curva_default,
     main = "Curva ROC del Modelo de Árbol",
     col = "blue", 
     lwd = 2,
     legacy.axes = TRUE,
     print.auc = TRUE,
...
# Agregar etiquetas y una línea diagonal
...
# Función para evaluar el rendimiento del árbol, la función recibe los híper-parámetros y un data set de entrenamiento y otro de validación, y devuelve un score, calculado a partir del modelo formado por los datos de input

evaluar_arbol <- function(maxdepth, minsplit, minbucket, train_set, val_set) {
  
  # Creamos nuestro modelo, dados los híper-parámetros
  modelo <- rpart(
  ...
  # Generamos predicciones con el set de validación
  ...
  # Devolvemos el score del modelo
...
result <- BayesianOptimization(
  ...
  bounds = limites, 
  init_points = 25, # Número de iteraciones aleatorias que hará el modelo
  n_iter = 5, # Número de variables de mejora que hará el modelo
)
```
```{r}

# Generamos una función para graficar como varía el performance a medida que aumentan las iteraciones
...
Por otro lado, graficamos la performance junto con las iteraciones de la optimización, para ver si hay algun valor donde el model converge.
...
```
En este gráfico, podemos observar que realizar 100 iteraciones no es necesario ya que aproximadamente a las 25 iteraciones se puede observar una convergencia al maximo valor del area bajo la curva de AUC-ROC que es ≈ 0.6818. Este trade-off entre tiempo de ejecución y performance del modelo es clave a la hora de establecer el valor de *n_iter*.
...
# Creamos el modelo utilizando los mejores híper-parámetros encontrados
mejor_arbol <- rpart(
  formula = popularity ~ ., 
  data = train_set, 
  method = "class", 
  control = rpart.control(
    maxdepth = mejores_hiperparametros[1], 
    minsplit = mejores_hiperparametros[2],
    minbucket = mejores_hiperparametros[3], 
    cp = 0,
    xval = 0
  )
)

# Graficamos el árbol para ver la diferencia en complejidad respecto al árbol inicial
