...
Decidimos utilizar un dataset de aproximadamente 75000 canciones de spotify con el objetivo de predecir si una cancion es popular o no en funcion del resto de las variables. Los datos fueron extraidos desde la pagina de Kaggle.
...
```{r}
library(readr)
spotify_data = read_csv("spotify_data.csv", show_col_types = FALSE)

table(is.na(spotify_data))
```


Una vez cargado, empieza la etapa de limpieza de los datos. Por ejemplo, lo primero que notamos fue que el dataset contenia mas de 50000 observaciones (73609). Por lo que hacemos un sampleo aleatorio de 50000 observaciones (el enunciado indica que el mismo no debe superar esta cantidad)


```{r}
set.seed(45963658)

if (nrow(spotify_data) >= 50000) {
  # Crear un vector de índices aleatorios
  indices <- sample(1:nrow(spotify_data), 50000)
  
  # Seleccionar las filas correspondientes a esos índices
  muestra <- spotify_data[indices, ]
...
# Convertir las variables 'key' y 'time_signature' a factores
muestra <- subset(muestra, select = -track_genre)
#muestra <- subset(muestra, select = -key)
#muestra <- subset(muestra, select = -time_signature)
```

```{r}
# Mapeo para la variable 'key'
key_mapping <- c("C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B")

# Convertir los valores de 'key' a palabras
muestra$key <- key_mapping[as.numeric(muestra$key) + 1]

# Mapeo para la variable 'time_signature'
time_signature_mapping <- c("0/4", "1/4", "2/4", "3/4", "4/4", "5/4")

# Convertir los valores de 'time_signature' a palabras
muestra$time_signature <- time_signature_mapping[as.numeric(muestra$time_signature) + 1]
```
...
# Graficar la función de densidad de instrumentalness
ggplot(muestra, aes(x = instrumentalness, fill=as.factor(popularity))) +
  geom_density(alpha = 0.4) +
  labs(title = "Función de Densidad de Instrumentalness",
       x = "Instrumentalness",
       y = "Densidad") +
  theme_minimal()
...
muestra$mode <- ifelse(muestra$mode == 1, 1, 0)
```
...
library(rpart.plot)
rpart.plot(arbol, box.palette = "lightblue")
...
test_set$predicted_class_popularity <- as.numeric(predictions_class) - 1
```
...
Accuracy, Precision, Recall y F1_score
...
matriz_de_confusion_optimizada = generar_matriz_confusion(test_set$popularity, test_set$predicciones_optimas)
...
muestra_numerica$popularity <- muestra_numerica$popularity
